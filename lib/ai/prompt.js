...
/**
 * Same inputs as buildSystemPrompt, but returns { stable, dynamic }.
 * `stable` changes only when shop data/settings change → safe to prompt-cache.
 * `dynamic` contains per-request content (date/time, customer, memory).
 */
export function buildSystemPromptParts({ shop, settings, customer, memory, knowledge, knowledgeSections, rules, dynamicContext, now }) {
  if (settings?.systemPromptOverride) return { stable: settings.systemPromptOverride, dynamic: "" };
  ...
  // Stable prefix: only changes when shop data/settings/KB change → prompt-cacheable.
  const stableSections = [
    // 1. AI Role + Shop Identity (no date/time — that lives in the dynamic part)
    _sectionIdentity(shop, settings, personality, lang),
    ...
    // 13. Campaigns from KB
    kbCampaigns || null,
    // 14. Rest of Knowledge Base
    kbFallback  ? _sectionKnowledge(kbFallback, lang) : null,
    kbOther     ? _sectionKnowledge(kbOther, lang)    : null,
    // 15. Working hours from KB (manual overrides; rarely needed with dynamic)
    kbWorkingHrs || null,
    // 16. Greeting/Closing
  ...
  // Dynamic suffix: per-request (current time, customer profile, memory).
  const dynamicSections = [
    lang === "en"
      ? `Current date/time: ${dateStr}, ${timeStr}.`
      : `Şu anki tarih/saat: ${dateStr}, saat ${timeStr}.`,
  ...
  return {
    ...
    dynamic: dynamicSections.filter(Boolean).join("\n\n"),
  };
}

// ── Sections ──────────────────────────────────────────────────────────────────

function _sectionIdentity(shop, settings, personality, lang) {
  if (lang === "en") {
    const lines = [
