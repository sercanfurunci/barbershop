...
/**
 * Score all slots against the customer profile so the AI recommends the best
 * fit instead of reading slots in arbitrary order. Deterministic, no AI cost.
 */
function rankSlots(result, customer, date) {
  if (!result?.results?.length) return result;

  const dayName = DAY_NAMES_TR[new Date(`${date}T12:00:00Z`).getUTCDay()];
  const preferredDay = customer?.preferredDays?.includes(dayName) ?? false;
  const prefHours = (customer?.preferredHours ?? []).map(h => parseInt(h, 10)).filter(Number.isFinite);

  const scored = [];
  for (const r of result.results) {
    for (const time of r.slots ?? []) {
      const hour = parseInt(time, 10);
      let score = 50;
      const reasons = [];
      if (customer?.favoriteBarber?.id === r.barberId) { score += 25; reasons.push("favorite barber"); }
      if (prefHours.some(ph => Math.abs(ph - hour) <= 1)) { score += 15; reasons.push("usual hour"); }
      if (preferredDay) { score += 5; reasons.push("usual day"); }
      scored.push({ time, barberId: r.barberId, barberName: r.barberName, score, reasons });
    }
  }
  if (!scored.length) return result;

  // Earliness bonus: soonest slots get up to +10
  scored.sort((a, b) => a.time.localeCompare(b.time));
  scored.forEach((s, i) => { s.score += Math.max(0, 10 - i * 2); });
...
export const handlers = {
  ...
  "availability.findAllSlots": async ({ shopId, serviceId, date }, ctx) => {
    const result = await getAllBarbersSlots({ shopId, serviceId, date });
    return rankSlots(result, ctx?.customer, date);
  },

  // ── booking.create ────────────────────────────────────────────────────────
  "booking.create": async ({ onBehalfOf, ...params }) => {
    // Confidence preflight: cheap checks createBooking doesn't surface as
    // clarification prompts. Low confidence → ask, don't book.
    const clarify = [];
    const digits = String(params.phone ?? "").replace(/\D/g, "");
    if (!/^(90)?0?5\d{9}$/.test(digits))            clarify.push("valid Turkish mobile number");
    if (!params.name || params.name.trim().length < 2) clarify.push("customer name");
    if (!/^\d{4}-\d{2}-\d{2}$/.test(params.date ?? "")) clarify.push("date (YYYY-MM-DD)");
    if (!/^\d{2}:\d{2}$/.test(params.time ?? ""))       clarify.push("time (HH:MM)");
    if (clarify.length) {
      return { ok: false, needsClarification: clarify,
               hint: "Ask the customer for the missing/invalid fields. Do NOT confirm a booking." };
    }
 ...
 */
export async function dispatch(handlerName, params, ctx) {
