...
export async function queueNotifications(appointmentId, event) {
  try {
    ...
    // Build messages/schedules before any DB work so we can skip early.
    const jobs = [];
    for (const channel of channels) {
      ...
      jobs.push({ channel, message, scheduledFor });
    }
    if (!jobs.length) return;

    // Dedup: batch-check all channels in one query instead of one per channel.
    const existingJobs = await prisma.notificationJob.findMany({
      where: {
        appointmentId,
        event,
        channel:  { in: jobs.map(j => j.channel) },
        status:   { in: ["PENDING", "PROCESSING", "SENT"] },
      },
      select: { channel: true },
    });
    const alreadyQueued = new Set(existingJobs.map(j => j.channel));

    const toCreate = jobs
      .filter(j => !alreadyQueued.has(j.channel))
      .map(j => ({
        shopId:        appt.shopId,
        appointmentId,
        channel:       j.channel,
    ...
    if (toCreate.length > 0) {
      await prisma.notificationJob.createMany({ data: toCreate });
    }
  } catch (err) {
...
export async function sendWhatsapp(phone, message, settings) {
  if (!settings.netgsmUser || !settings.netgsmPassword) {
    throw new Error("Netgsm credentials not configured");
  }
...
// settings must be pre-fetched by caller to avoid N+1.
...
export async function processQueue(limit = 20) {
  ...
  if (jobs.length === 0) return { sent: 0, failed: 0, skipped: 0 };

  // Batch-load all shop settings in one query instead of one per job.
  const shopIds = [...new Set(jobs.map(j => j.shopId))];
  const settingsRows = await prisma.notificationSettings.findMany({
    where: { shopId: { in: shopIds } },
  });
  const settingsMap = new Map(settingsRows.map(s => [s.shopId, s]));
  ...
  for (const job of jobs) {
    ...
    try {
      await processJob(job, settingsMap.get(job.shopId));
