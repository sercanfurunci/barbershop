...
function getScheduledFor(event, appt, settings) {
  ...
  const apptMs = Date.UTC(y, mo - 1, d, h - 3, mi); // UTC offset for Istanbul (UTC+3)
...
// Cancel pending reminders/followup when appointment is cancelled
export async function cancelPendingJobs(appointmentId) {
  await prisma.notificationJob.updateMany({
    where: {
      appointmentId,
      status: "PENDING",
      event:  { in: ["REMINDER_48H", "REMINDER_3H", "FOLLOWUP"] },
    },
...
async function sendWhatsapp(phone, message, settings) {
  ...
  const res = await fetch("https://api.netgsm.com.tr/whatsapp/send/", {
    ...
    body:    JSON.stringify({
      usercode: settings.netgsmUser,
      password: settings.netgsmPassword,
      gsmno:    "90" + phone, // Netgsm WA requires +90 prefix without "+"
      message,
    }),
  });
