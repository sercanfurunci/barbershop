...
export async function verifyWebhook(request) {
  ...
  // Fail closed: once IYZICO_API_KEY/SECRET are set the route MUST verify
  // signatures. Returning null here would have the webhook handler ack 200
  // for unsigned payloads — a one-flag-flip route to forged subscription state.
  // TODO: validate signature header, parse payload, return:
  //   { event, shopId, eventId, invoiceData }
  throw new Error("iyzico webhook verification not implemented — see lib/payments/iyzico.js");
}
