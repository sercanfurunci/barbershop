...
export async function rateLimit(key, { limit = 5, windowMs = 10 * 60 * 1000 } = {}) {
  ...
  try {
    ...
    // ponytail: no structured logger here — this is infrastructure, not a request handler
    process.stderr.write(`[rateLimit] Upstash error, falling back to local: ${err.message}\n`);
