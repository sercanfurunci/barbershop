...
export async function getDefaultShopId() {
  ...
  try {
    ...
    console.error(`[getDefaultShopId] DB error for shop "${DEFAULT_SHOP_SLUG}":`, err.message);
    throw err;
  }
  if (!shop) {
    console.error(`[getDefaultShopId] shop "${DEFAULT_SHOP_SLUG}" not found — DATABASE_URL set? DB migrated?`);
    throw new Error(`Default shop "${DEFAULT_SHOP_SLUG}" not found. Run prisma db push and seed.`);
  }
