...
export async function createBooking({
  ...
  // ponytail: skip for non-web channels — phone IS the identity on WhatsApp/AI chat
  const normalizedSrc = normalizeSource(source);
  const NON_WEB_SOURCES = new Set(["AI_CHAT", "WHATSAPP", "INSTAGRAM", "VOICE", "MESSENGER"]);
  if (!bookedByUserId && !NON_WEB_SOURCES.has(normalizedSrc)) {
  ...
  // Invalidate customer context cache so next AI message sees the new appointment
  if (phone) invalidateCustomerContext(shopId, phone);
