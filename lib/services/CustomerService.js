...
export async function findOrCreateClient(db, { shopId, name, phone, email }, existing = null) {
  if (existing) {
    if (email && email !== existing.email) {
      return db.client.update({ where: { id: existing.id }, data: { email } });
    }
    return existing;
  }
  // Always store normalized phone to prevent duplicate clients across different raw formats
  const storedPhone = normalizePhone(phone) ?? phone;
  return db.client.create({
    data: { shopId, name: name.trim(), phone: storedPhone, email: email || null },
  });
}
